...
function run_tests {
  ...
  if [ setup.cfg -nt guts.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='guts/*' --omit='guts/openstack/common/*' -d covhtml -i
  fi
...
function run_pep8 {
  ...
  ${wrapper} tools/check_exec.py guts || exit 1
}
