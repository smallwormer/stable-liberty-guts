...
# Note(Bharat): E1103 is error code related to partial type inference
ignore_codes = ["E1103"]
# Note(Bharat): the error message is the pattern of E0202. It should be ignored
# for guts.tests modules
# Note(Bharat): the second error message is the pattern of [E0611].
# It should be ignored because use six module to keep py3.X compatibility.
# Note(Bharat): the third error message is for SQLAlchemy update() calls
# in DB schema migrations.
# Note(Bharat): the fourth and fifth error messages are for the code [E1101].
# They should be ignored because 'sha256' and 'sha224' are functions in
# 'hashlib'.
# Note(Bharat): the sixth error message is for SQLAlchemy rename calls in
# DB migration(033_add_encryption_unique_key).
ignore_messages = ["An attribute affected in guts.tests",
...
# Note(Bharat): we ignore all errors in openstack.common because it should be
# checked elsewhere. We also ignore guts.tests for now due to high false
# positive rate.
ignore_modules = ["guts/openstack/common/", "guts/tests/"]

# Note(Bharat): E0213, E1101, and E1102 should be ignored for only
# guts.object modules. E0213 and E1102 are error codes related to
...
# Note(Bharat): The error messages are for codes [E1120, E1101] appearing in
# the guts code base using objects. E1120 is an error code related no value
...
objects_ignore_modules = ["guts/objects/"]
