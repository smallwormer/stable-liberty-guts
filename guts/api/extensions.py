...
from guts.i18n import _LE, _LI, _LW
import guts.policy
...
class ControllerExtension(object):
    """Extend core controllers of guts OpenStack API.

    Provide a way to extend existing guts OpenStack API core
    controllers.
    """
...
class ResourceExtension(object):
    """Add top level resources to the OpenStack API in guts."""
...
def extension_authorizer(api_name, extension_name):
    def authorize(context, target=None, action=None):
        ...
        guts.policy.enforce(context, act, target)
    return authorize
