...
Guidelines for writing new hacking checks
 ...
 - Use only for Guts specific tests. OpenStack general tests
 ...
 - Add test cases for each new rule to
   guts/tests/test_hacking.py

"""

# NOTE(Bharat): Ignore N323 pep8 error caused by importing guts objects
UNDERSCORE_IMPORT_FILES = ['./guts/objects/__init__.py']
...
# NOTE(Bharat): When other oslo libraries switch over non-namespaced
...
class CheckLoggingFormatArgs(BaseASTChecker):
    ...
    LOG.debug("Migration %s caught fire and is at %d degrees C and climbing.",
              ('migration1', 500))
...
def no_test_log(logical_line, filename, noqa):
    if "guts/tests" not in filename or noqa:
        return
    # Skip the "integrated" tests for now
    if "guts/tests/unit/integrated" in filename:
        return
